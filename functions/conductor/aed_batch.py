...
def driver(event, context):
    ...
    print('Pattern matching name: ' + event['name'])
    print('Playlist ID: ' + str(event['playlist_id']))
    citizen = event.get('citizen_scientist', 0)
    ...
    param_string = '{"threshold": '+str(float(event['threshold'])) + ', "N": '+str(event['N']) + '}'
                        
    ins = job_params.insert().values(name=event['name'],
                                  project_id=proj_id,
                                  job_id=job_id,
                                  timestamp=dt.datetime.now(),
                                  species_id=templ_qres[0][0],
                                  songtype_id=templ_qres[0][1],
                                  parameters=param_string,
                                  playlist_id=event['playlist_id'],
                                  template_id=event['template_id'],
                                  citizen_scientist=citizen)
    result = session.execute(ins)
